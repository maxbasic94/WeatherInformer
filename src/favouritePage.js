...
function createFavCityDiv(favCity) {
  ...
  favCityDiv.addEventListener("click", (event) => {
    ...
    createHomePage(
      `http://api.weatherapi.com/v1/forecast.json?key=0ca217e793694cf3b27105654211511&q=${cityName}&days=4&aqi=no&alerts=no`
    );
  });
  return favCityDiv;
}
    ...
    li.addEventListener("click", (event) => {
      if (event.target.tagName === "svg" || event.target.tagName === "path") {
        ...
        if (svgStar.getAttribute("fill") === "white") {
          ...
          if (
            !favouriteCitiesArr.includes(
              event.currentTarget.textContent.split(",")[0]
            )
          ) {
            favouriteCitiesArr.push(
              event.currentTarget.textContent.split(",")[0]
            );
          }
        } else {
          svgStar.setAttribute("fill", "white");
          const index = favouriteCitiesArr.indexOf(
            event.currentTarget.textContent.split(",")[0]
          );
          if (index > -1) {
        ...
        createHomePage(
          `http://api.weatherapi.com/v1/forecast.json?key=0ca217e793694cf3b27105654211511&q=${city}&days=4&aqi=no&alerts=no`
        );
      }
